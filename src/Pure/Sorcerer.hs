...
{-# INLINE read #-}
read :: forall ev m ag. (MonadIO m, Typeable ev, Typeable ag, ToJSON ev, FromJSON ev, Streamable ev, Manageable ev, Ord (Stream ev), Aggregable ev ag) => Stream ev -> m (Maybe ag)
read s = liftIO do
  mv <- newEmptyMVar
...
{-# INLINE write #-}
write :: forall ev m. (MonadIO m, Typeable ev, Streamable ev, Manageable ev, ToJSON ev, FromJSON ev, Ord (Stream ev)) => Stream ev -> ev -> m ()
write s ev = liftIO do
  dispatchWith (startManagerWithBuilders @ev) s (Write ev)

{-# INLINE writeMany #-}
writeMany :: forall ev m. (MonadIO m, Typeable ev, Streamable ev, Manageable ev, ToJSON ev, FromJSON ev, Ord (Stream ev)) => Stream ev -> [ev] -> m ()
...
{-# INLINE transact #-}
transact :: forall ev m ag. (MonadIO m, Typeable ag, Typeable ev, Streamable ev, Manageable ev, Aggregable ev ag, ToJSON ev, FromJSON ev, Ord (Stream ev)) => Stream ev -> ev -> m (Maybe (Maybe ag))
...
{-# INLINE observe #-}
observe :: forall ev m ag. (MonadIO m, Typeable ag, Typeable ev, Streamable ev, Manageable ev, Aggregable ev ag, ToJSON ev, FromJSON ev, Ord (Stream ev)) => Stream ev -> ev -> m (Maybe ag,Maybe (Maybe ag))
